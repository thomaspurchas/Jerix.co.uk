...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for blob in orm.ParentBlob.objects.all():
            docs = blob.documents.all()
            docs.update(file_type=blob.file_type)

        for blob in orm.DerivedBlob.objects.all():
            docs = blob.documents.all()
            docs.update(file_type=blob.file_type)
    ...
    def backwards(self, orm):
        ...
        for doc in orm['files.Document'].objects.all():
            blob = doc._blob
            blob.file_type = doc.file_type
            blob.save()

        for doc in orm['files.DerivedDocument'].objects.all():
            blob = doc._blob
            blob.file_type = doc.file_type
            blob.save()
